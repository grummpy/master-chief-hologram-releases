...
const CONTROL_MODES = new Set(['revision', 'pose', 'faceid', 'canny', 'depth', 'instantid', 'hybridid', 'tile', 'poselora']);
...
function normalizeShot(input = {}, existing = {}) {
  ...
  return {
    ...
    identityReference: cleanText(input.identityReference ?? existing.identityReference, 1000), styleReference: cleanText(input.styleReference ?? existing.styleReference, 1000),
    poseReference: cleanText(input.poseReference ?? existing.poseReference, 1000), compositionReference: cleanText(input.compositionReference ?? existing.compositionReference, 1000),
    depthReference: cleanText(input.depthReference ?? existing.depthReference, 1000), lightingReference: cleanText(input.lightingReference ?? existing.lightingReference, 1000),
