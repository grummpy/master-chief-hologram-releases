...
(() => {
  ...
  if (purposeSelect && ![...purposeSelect.options].some(option => option.value === 'hybridid')) purposeSelect.add(new Option('Hybrid Identity · FaceID + InstantID precision', 'hybridid'), 2);
  ...
  if (purposeSelect && ![...purposeSelect.options].some(option => option.value === 'depth')) purposeSelect.add(new Option('Depth · preserve spatial layout and perspective', 'depth'));
  ...
  async function loadArtifacts() {
    ...
    if (catalog.detected?.poseExtractor) ready.push(`automatic pose extraction ready (${catalog.detected.poseExtractorNodes.join(', ')})`);
    else if (catalog.detected?.controlNet && catalog.controlnets?.some(name => /openpose/i.test(name))) ready.push('OpenPose model ready · automatic pose extraction not installed; use a prepared skeleton map');
    ...
    const depthWorkflow = catalog.workflows.find(item => item.id === 'sdxl-depth-control-v1');
    if (depthWorkflow?.readiness === 'ready') ready.push('automatic depth and perspective control ready');
    else if (catalog.detected?.depthControl && catalog.controlnets?.includes('sdxl-depth.safetensors')) ready.push('depth control detected · auxiliary Depth Anything weights still gated');
    if (catalog.controlnets?.includes('sdxl-tile.safetensors')) ready.push('Tile detail/layout control ready');
    if (catalog.controlnets?.includes('control-lora-openposeXL2-rank256.safetensors')) ready.push('lower-memory OpenPose Control-LoRA ready');
    ...
    if (catalog.capabilities?.flux) ready.push('FLUX.1 Dev FP8 local image generation ready · one-image safe mode');
  ...
  get('identityLockPreset').onclick = () => {
    ...
    const hybridReady = [...get('shotWorkflow').options].some(option => option.value === 'sdxl-hybrid-identity-v1' && !option.disabled);
    get('shotControlMode').value = hybridReady ? 'hybridid' : 'instantid';
    get('shotWorkflow').value = hybridReady ? 'sdxl-hybrid-identity-v1' : 'sdxl-instantid-v1';
    get('shotReferenceStrength').value = hybridReady ? '.85' : '1'; get('shotReferenceStrengthValue').value = get('shotReferenceStrength').value;
    get('shotFaceIdV2Strength').value = '1.05'; get('shotFaceIdLoraStrength').value = '.45';
    get('shotInstantIdControlStrength').value = hybridReady ? '.78' : '.82'; get('shotInstantIdNoise').value = hybridReady ? '0' : '.25';
    ...
    invalidatePreflight();
    get('sceneCoachAdvice').textContent = hybridReady ? 'Hybrid Identity Precision loaded from the verified FaceID Plus v2 + InstantID test. Preserve visible identity anchors such as glasses in the prompt, then review the exact job.' : 'Identity Lock loaded from the verified InstantID test. Keep identity traits in Continuity locks, describe the transformation in the positive prompt, then review the exact job.';
  ...
  const sceneRecipes = {
    ...
    twoperson: { title: 'Two-person blocking', pose: 'two clearly separated subjects with unambiguous left/right positions, distinct gestures, visible hands, natural eyelines', camera: 'medium-wide two-shot, 50mm lens, eye-level camera, both subjects fully readable', width: 1216, height: 832, batch: 4, advice: 'Two-person blocking loaded. Name Subject A and Subject B and describe each position separately.' },
    product: { title: 'Product hero shot', pose: 'single product centered with clean silhouette and accurate geometry', camera: 'commercial hero angle, 85mm product lens, controlled perspective, sharp focus', lighting: 'large soft key, edge highlights, clean gradient background, premium studio reflections', width: 1024, height: 1024, advice: 'Product recipe loaded. Describe materials, brand colors, surface, and required label visibility.' },
    environment: { title: 'Environment study', pose: 'small foreground figure only if scale is needed', camera: 'wide establishing shot, 24mm lens, deep layered composition, strong foreground-midground-background separation', lighting: 'time-of-day motivated atmospheric light, volumetric depth, readable focal path', width: 1216, height: 832, batch: 4, advice: 'Environment recipe loaded. Describe geography, architecture, weather, time, and the visual focal path.' }
  };
  const recipeBar=document.querySelector('.scene-recipe-actions');
  for(const [key,label] of [['twoperson','Two-person blocking'],['product','Product shot'],['environment','Environment']]) if(recipeBar&&!recipeBar.querySelector(`[data-scene-recipe="${key}"]`)){const button=document.createElement('button');button.type='button';button.dataset.sceneRecipe=key;button.textContent=label;recipeBar.append(button)}
