...
secureHandle('experience-status',()=>({state:workspaceExperience.snapshot(),templates:workspaceExperience.templates(),artifactTypes:workspaceExperience.artifactTypes()}));
secureHandle('experience-preset-save',(_event,payload)=>workspaceExperience.savePreset(payload));
secureHandle('experience-compare',(_event,payload)=>workspaceExperience.compare(payload));
secureHandle('experience-compare-decision',(_event,payload)=>workspaceExperience.decide(payload?.id,payload?.artifactId,payload?.decision,payload?.reason));
secureHandle('experience-onboard',(_event,payload)=>workspaceExperience.onboard(payload));
secureHandle('experience-filter-models',(_event,payload)=>workspaceExperience.filterModels(payload?.models,payload?.filters));
secureHandle('experience-dependency',(_event,payload)=>workspaceExperience.dependency(payload?.feature,payload?.online));
secureHandle('experience-activity',(_event,payload)=>workspaceExperience.recordActivity(payload));
secureHandle('telemetry-status',()=>localTelemetry.snapshot());
secureHandle('telemetry-capture',(_event,payload)=>localTelemetry.setCapture(payload?.projectId,payload?.enabled));
secureHandle('telemetry-trace',(_event,payload)=>localTelemetry.trace(payload));
secureHandle('telemetry-golden-add',(_event,payload)=>localTelemetry.addGolden(payload));
secureHandle('telemetry-evaluate',(_event,payload)=>localTelemetry.evaluate(payload));
secureHandle('telemetry-canary',(_event,payload)=>localTelemetry.canary(payload?.route,payload?.candidate,payload?.promoted,payload?.percent));
secureHandle('telemetry-replay',(_event,payload)=>localTelemetry.replay(payload?.traceId));
secureHandle('telemetry-scorecard',()=>({capabilities:localTelemetry.scorecard()}));
secureHandle('telemetry-playground',(_event,payload)=>localTelemetry.playground(payload));
...
secureHandle('artifact-metadata', (_event, relativePath) => {
  ...
  const embedded = path.extname(artifactPath).toLowerCase() === '.png' ? require('./png-workflow-metadata').parsePngWorkflowMetadata(bytes) : null;
  const settings = job?.parameters || embedded?.settings || null;
  return { filename: path.basename(artifactPath), path: relative, bytes: stat.size, modifiedAt: stat.mtime.toISOString(), sha256: require('crypto').createHash('sha256').update(bytes).digest('hex'), job: job || null, embedded, recoverableSettings: settings };
});
