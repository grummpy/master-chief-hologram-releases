...
for (const token of ['id="app"', 'id="holoStage"', 'id="holoImg"', 'id="holoStatus"', 'id="visualModeBtn"', 'id="micBtn"', 'id="sendBtn"', 'id="autocompleteList"', 'aria-live="polite"']) if (!html.includes(token)) fail(`index.html is missing ${token}`);
for (const token of ['#app[data-theme=day]', '.holo-stage', '.holo-stage img', 'object-fit:cover', '.holo-status', '.scanlines', '.autocomplete-list', '@media(prefers-reduced-motion:reduce)', 'button:focus-visible']) if (!css.includes(token)) fail(`styles.css is missing ${token}`);
for (const name of ['holographic-command-officer-3d-v1.png', 'night_idle.jpg', 'night_listening.jpg', 'night_thinking.jpg', 'night_speaking.jpg', 'night_success.jpg', 'night_alert.jpg', 'night_wave.jpg', 'day_idle.jpg', 'day_ready.jpg', 'day_thinking.jpg', 'day_wave.jpg', 'day_success.jpg']) if (!renderer.includes(name)) fail(`renderer state map no longer references ${name}`);
