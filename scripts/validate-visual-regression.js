...
for (const token of ['id="app"', 'id="holoStage"', 'id="holoImg"', 'id="holoStatus"', 'id="micBtn"', 'id="sendBtn"', 'aria-live="polite"']) if (!html.includes(token)) fail(`index.html is missing ${token}`);
for (const token of ['#app[data-theme=day]', '.holo-stage', '.holo-stage img', 'object-fit:cover', '.holo-status', '.scanlines', '@media(prefers-reduced-motion:reduce)', 'button:focus-visible']) if (!css.includes(token)) fail(`styles.css is missing ${token}`);
