...
| Scheduled reminders and monitoring | Partial | Durable reminders and quiet state-change monitors for local Ollama and private-LAN ComfyUI include native notifications, pause/resume/cancel, history, restart recovery, and Ollama tools. Cloud/account monitoring is not yet implemented. |
...
1. Extend conditional monitoring only through connector-specific, read-only contracts with explicit user selection; do not introduce generic URL polling.
