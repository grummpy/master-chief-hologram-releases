...
function buildCompanion() {
  ...
  const eyeMaterial = material(0x85f5ff, 1.6); const eyeL = part(new THREE.SphereGeometry(.052, 10, 8), eyeMaterial, [-.12, 1.33, .42]); const eyeR = part(new THREE.SphereGeometry(.052, 10, 8), eyeMaterial, [.12, 1.33, .42]); const badge = part(new THREE.CircleGeometry(.075, 12), cyan, [0, .82, .49]);
  root.add(head, hairCap, eyeL, eyeR, badge, cylinder(.42, .56, .85, 0x102c55, [0, .62, .05]), part(new THREE.BoxGeometry(.72, .08, .12), silver, [0, .88, .1]));
...
function init() {
  ...
  } catch (error) { console.error('Interactive 3D scene initialization failed', error); enabled = false; canvas.hidden = true; stage.dataset.threeError = String(error?.message || error); document.getElementById('holoStatus').textContent = `3D unavailable: ${stage.dataset.threeError}`; }
}
