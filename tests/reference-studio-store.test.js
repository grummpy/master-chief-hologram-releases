...
test('Reference Studio persists identity data and a multi-shot queue atomically', () => {
  ...
  try {
    ...
    const project = store.saveProject({ title: 'Nova continuity', subject: { name: 'Commander Nova', identityLock: 'silver hair and blue eyes', adultConfirmed: true } });
    ...
    assert.equal(state.projects[0].subject.adultConfirmed, true);
