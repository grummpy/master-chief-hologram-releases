...
function cloneAndFillWorkflow(template, values) {
  const replacements = new Map([
    ...
    ['{{CANNY_LOW}}', Number.isFinite(values.cannyLow) ? Math.min(.99, Math.max(.01, values.cannyLow)) : .35],
    ['{{CANNY_HIGH}}', Number.isFinite(values.cannyHigh) ? Math.min(.99, Math.max(.01, values.cannyHigh)) : .75],
    ['{{INSTANTID_CONTROL_STRENGTH}}', Number.isFinite(values.instantIdControlStrength) ? Math.min(10, Math.max(0, values.instantIdControlStrength)) : .8],
    ['{{INSTANTID_NOISE}}', Number.isFinite(values.instantIdNoise) ? Math.min(1, Math.max(0, values.instantIdNoise)) : 0],
