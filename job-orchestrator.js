...
function createJobOrchestrator(file) {
  ...
  return {submit,event,setRoute,setPlan,checkpoint,addArtifact,action,fail,get:id=>clone(find(id)||null),list:(limit=100)=>clone(state.jobs.slice(0,Math.min(1000,Math.max(1,Number(limit)||100)))),filePath:file};
}
